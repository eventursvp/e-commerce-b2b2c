...
exports.getListOfVendors = async (req, res, next) => {
    try {
        const { userId } = req.body
        const { loginUser } = req
        if (!userId) {
        ...
        const vendors = await Vendor.find({isReject:false,isBlocked:false})
        if (!vendors || vendors.length === 0) {
            return res.status(404).send({ status: 1, message: "Empty set.", data: [] })
        }
        return res.status(200).send({ status: 1, message: "Users found.", data: vendors })
    } catch (error) {
        console.log('error =>', error);
        return res.status(500).send({ status: 0, message: "Something went wrong", error });
    }
