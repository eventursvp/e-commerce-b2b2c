...
exports.verifyVendorEmail = async (req, res, next) => {
    try {
        ...
        const decoded = jwt.verify(token, process.env.JWT_TOKEN);
        if (decoded) {
            const vendor = await Vendor.findOne({ email: decoded.email.toLowerCase() })
            if (vendor) {
                if (vendor?.emailVerified) {
                    return res.status(200).send({ status: 0, message: 'Email already verified' });

                } else {
                    const updateVendor = await Vendor.findOneAndUpdate({ email: decoded.email }, { emailVerified: true }, { new: true })
                    if (!updateVendor) {
                        return res.status(500).send({ status: 0, message: "Email not verified please try again" })
                    }
                    await createApplicationLog("Auth", "verify vendor email", {}, {}, vendor?._id)
                    return res.status(200).send({ status: 1, message: "Email verified successfully" });
                }
            } else {
                return res.status(400).send({ status: 0, message: "User not found." });
