...
exports.changeVendorPassword = async (req, res) => {
    try {
        const { vendorId, oldPassword, newPassword, confirmPassword } = req.body;
        const { loginUser } = req
        if (!vendorId) {
        ...
        if (validPassword) {
            ...
            const hashedPassword = await hashPassword(newPassword);
            const result = await Vendor.findByIdAndUpdate(vendorId, { password: hashedPassword }, { new: true });
            if (!result) {
                return res.status(500).send({ status: 0, message: "Password not changed please try again." })
            }
            await createApplicationLog("Auth", "change vendor password", {}, {}, loginUser?._id)
            return res.status(201).send({ status: 1, message: "Your password changed." })
        }
