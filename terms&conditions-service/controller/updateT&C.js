...
exports.updateProductTermsAndCondition = async(req,res)=>{
    try {
        const{title,description,addedBy,termsAndConditionId,productId} = req.body;
        ...
        if (!(title || description && addedBy && termsAndConditionId && productId)) {
            return res.status(409).send({ status: 0, message: "Title and description are required" });
        }

        const data = await TermsAndConditions.findOneAndUpdate(
            { _id: termsAndConditionId, addedBy: addedBy, isDeleted: false,productId:productId },
            { $set: { title: title, description: description } },
            { new: true }
        );

        if (!data) {
            return res.status(404).send({ status: 0, message: "Error updating product terms and conditions", data: [] });
        }

        await createApplicationLog("TermsAndCondition", "update product terms and condition", {}, {}, addedBy);
